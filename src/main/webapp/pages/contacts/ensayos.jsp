...
<!-- ensayos.jsp: generated by stajanov code generator -->
<f:loadBundle basename="com.autentia.intra.resources.messages" var="msg"/>
<i:location place="ensayos" msg="${msg}"/>
...
<f:view>
    ...
    <h:form id="ensayos">
        ...
        <%-- Header --%>
        <i:titleBar msg="${msg}">
            <h:commandLink action="#{ensayoBean.search}">
                <h:graphicImage rendered="#{ ensayoBean.search.searchActive}" title="#{msg.entityActions_filtered}"
                                value="/img/search_applied.gif" styleClass="titleImg"/>
                <h:graphicImage rendered="#{!ensayoBean.search.searchActive}" title="#{msg.entityActions_search}"
                                value="/img/search.gif" styleClass="titleImg"/>
            </h:commandLink>
            <h:commandLink action="#{ensayoBean.reset}">
                <h:graphicImage rendered="#{ensayoBean.search.searchActive}" title="#{msg.entityActions_reset}"
                                value="/img/eraser.png" styleClass="titleImg"/>
            </h:commandLink>
            <h:commandLink action="#{ensayoBean.create}" rendered="#{ensayoBean.createAvailable}">
        ...
        <div class="abcPager">
            ...
            <a:abcPager styleClass="abcPagerLetter" selectedStyleClass="abcPagerSelectedLetter"
                        value="#{ensayoBean.letter}" action="#{ensayoBean.letterClicked}"
                        allowUnselect="true" unselectImage="/img/no_funnel.png"/>
        </div>


        <%-- List of ensayos --%>
        <t:dataTable id="list" var="ensayo" value="#{ensayoBean.all}" preserveDataModel="false"
                     ...
                     columnClasses="listCmdCell,listEnsayoName,listEnsayoCost"
                     sortColumn="#{ensayoBean.sortColumn}" sortAscending="#{ensayoBean.sortAscending}"
            ...
            <h:column>
                ...
                <t:commandLink action="#{ensayoBean.detail}" immediate="true">
                    <f:param name="id" value="#{ensayo.id}"/>
            ...
            <h:column>
                <f:facet name="header">
                    <t:commandSortHeader styleClass="listHeader" columnName="name">
                        ...
                        <f:verbatim>${msg['ensayo.name']}</f:verbatim>
                    </t:commandSortHeader>
                </f:facet>
                ...
                <t:commandLink action="#{ensayoBean.detail}" immediate="true">
                    <f:param name="id" value="#{ensayo.id}"/>
                    <%-- String field --%>
                    <h:outputText value="#{ensayo.name}"/>
                </t:commandLink>
            ...
            <h:column>
                <f:facet name="header">
                    <t:commandSortHeader styleClass="listHeader" columnName="cost">
                        ...
                        <f:verbatim>${msg['ensayo.cost']}</f:verbatim>
                    </t:commandSortHeader>
                </f:facet>
                ...
                <t:commandLink action="#{ensayoBean.detail}" immediate="true">
                    <f:param name="id" value="#{ensayo.id}"/>
                    <%-- String field --%>
                    <h:outputText value="#{ensayo.cost}"/>
                </t:commandLink>
